...
`;var ro=Fe,Se=!1;function ao(){if(Se||typeof document>"u")return;Se=!0;let e=document.createElement("style");e.id="color-is-box-style",e.textContent=Ve,document.head.appendChild(e)}function Fe(e,o={}){let t=o.size??300,n=o.controls??!0,i=o.showInputs??!1,c=o.showModeToggle??!1,d=o.showCorners??!1,a={mode:()=>s,switchMode:r=>S(r),onHexInput:r=>{let l=ue(r);l?(b=Z(w?{r:255-l.r,g:255-l.g,b:255-l.b}:l,s),u={x:Math.max(u.x,b.x),y:Math.max(u.y,b.y),z:Math.max(u.z,b.z)},g(),N(),B()):N()},onChannelInput:(r,l,h)=>{let M=Math.max(0,Math.min(h,l)),L=["x","y","z"],v=M/h;if(w){let E={...b,[L[r]]:v},D=X(E,s);b=Z({r:255-D.r,g:255-D.g,b:255-D.b},s)}else b={...b,[L[r]]:v};v>u[L[r]]&&(u={...u,[L[r]]:v}),g(),N(),B()},getRgbForCopy:()=>X(b,s),onRandom:r=>p(r),onReset:()=>p({r:0,g:0,b:0})},s=o.mode??"rgb",x=o.initialColor?Z(o.initialColor,s):{x:.7,y:.4,z:.85},u={x:1,y:1,z:1},b={...x},C=0,m=new Set;ao();let y=document.createElement("div");y.className="box-picker";let k=document.createElement("canvas");k.style.cursor="grab",y.appendChild(k);let z=Me(k,t),F=null,I=document.createElement("div");I.className="box-picker-controls",F=document.createElement("div"),F.className="box-picker-swatch",I.appendChild(F),y.appendChild(I),(i||c||d)&&Promise.resolve().then(()=>(He(),ze)).then(r=>{r.createControls(I,a,{showInputs:i,showModeToggle:c,showCorners:d})}).catch(()=>{}),e.appendChild(y);let T=Te(k,()=>u,r=>{u=r},()=>b,(r,l)=>{b=r,C=l,g(),N()},()=>C,()=>z.scale,()=>z.center,B),w=!1,R=!0;k.addEventListener("mouseenter",()=>{R=Math.random()<.5,B()}),k.addEventListener("mouseleave",()=>{R=Math.random()<.5,B()}),k.addEventListener("dblclick",()=>{w=!w,be(w),g(),N(),B()});function S(r){if(r===s)return;let l=X(b,s),h={...b},M={...u};s=r;let L=Z(l,s),v={x:1,y:1,z:1};b=L,u=v,O(h,L,M,v,300),N()}let H=null;function O(r,l,h,M,L){H!==null&&cancelAnimationFrame(H);let v=performance.now();function E(D){let q=D-v,J=Math.min(1,q/L),K=1-Math.pow(1-J,3);b={x:r.x+(l.x-r.x)*K,y:r.y+(l.y-r.y)*K,z:r.z+(l.z-r.z)*K},u={x:h.x+(M.x-h.x)*K,y:h.y+(M.y-h.y)*K,z:h.z+(M.z-h.z)*K},U(),g(),J<1?H=requestAnimationFrame(E):H=null}H=requestAnimationFrame(E)}let P=!1;function B(){P||(P=!0,requestAnimationFrame(()=>{P=!1,U()}))}function U(){ke(z,u,b,C,s,T.state,R)}function j(r,l,h){return Math.round(r+(l-r)*h)}function G(r,l){let h=l>0?255:0,M=Math.abs(l);return W({r:j(r.r,h,M),g:j(r.g,h,M),b:j(r.b,h,M)})}function $(r,l){let h=ue(l)||{r:128,g:128,b:128},M=G(h,.35),L=G(h,0),v=G(h,-.35);r.innerHTML=`<svg viewBox="0 0 48 48" width="64" height="64" aria-hidden="true"><defs><filter id="sw-mssllzpn" x="-60%" y="-60%" width="220%" height="220%"><feDropShadow dx="0" dy="1" stdDeviation="1.2" flood-color="#000" flood-opacity="0.32"/></filter></defs><g filter="url(#sw-mssllzpn)"><path d="M24 5 L42 14 L24 23 L6 14 Z" fill="${M}"/><path d="M6 14 L24 23 L24 45 L6 36 Z" fill="${L}"/><path d="M42 14 L24 23 L24 45 L42 36 Z" fill="${v}"/><path d="M24 23 L24 45 M6 14 L6 36 M42 14 L42 36" stroke="rgba(0,0,0,.18)" stroke-width="1" fill="none"/></g></svg>`,r.style.backgroundColor="transparent"}function re(r){try{navigator.clipboard.writeText(r).catch(()=>{})}catch{}}function ae(r){r&&(r.style.borderColor="#4ade80",r.style.boxShadow="0 0 0 2px rgba(74,222,128,.35)",setTimeout(()=>{r.style.borderColor="",r.style.boxShadow=""},500))}function Y(){let r=X(b,s);return w?{r:255-r.r,g:255-r.g,b:255-r.b}:r}function N(){if(!n)return;let r=Y(),l=W(r);F&&$(F,l);let h=y.querySelector(".box-picker-hex");h&&(h.value=l),f(),y._updateModeButtons&&y._updateModeButtons()}function f(){if(!n)return;let r=oe[s],l=w?Z(Y(),s):b,h=pe(l,s),M=y.querySelectorAll(".box-picker-channel input"),L=y.querySelectorAll(".box-picker-channel label");for(let v=0;v<M.length;v++)L[v].textContent=r[v],L[v].style.color="",L[v].style.textShadow="none",M[v].value=String(h[v])}function g(){let r=Y(),l={rgb:r,hsb:te(r),oklch:ne(r),hex:W(r)};for(let h of m)h(l)}function A(){let r=X(b,s);return{rgb:r,hsb:te(r),oklch:ne(r),hex:W(r)}}N(),U();let p=r=>{b=Z(r,s),u={x:Math.max(u.x,b.x),y:Math.max(u.y,b.y),z:Math.max(u.z,b.z)};let l=V(b,z.scale,z.center);C=-1;for(let h=_.length-1;h>=0;h--)if(Q(h,l,u,z.scale,z.center)){C=h;break}g(),N(),B()};return{getColor:A,getMode:()=>s,setColor:p,setMode(r){S(r)},on(r,l){m.add(l)},off(r,l){m.delete(l)},destroy(){T.destroy(),H!==null&&cancelAnimationFrame(H),e.removeChild(y)}}}return _e(io);})();
