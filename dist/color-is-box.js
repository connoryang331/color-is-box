...
`;var Mt=jo,Lo=!1;function Wo(){if(Lo||typeof document>"u")return;Lo=!0;let e=document.createElement("style");e.id="color-is-box-style",e.textContent=So,document.head.appendChild(e)}function jo(e,o={}){let t=o.size??300,r=o.controls??!0,l=o.showInputs??!1,a=o.showModeToggle??!1,c=o.showCorners??!1,s={mode:()=>n,switchMode:i=>ne(i),onHexInput:i=>{let f=Ge(i);f?(b=de(W?{r:255-f.r,g:255-f.g,b:255-f.b}:f,n),m={x:Math.max(m.x,b.x),y:Math.max(m.y,b.y),z:Math.max(m.z,b.z)},ee(),_(),L()):_()},onChannelInput:(i,f,v)=>{let E=Math.max(0,Math.min(v,f)),D=["x","y","z"],B=E/v;if(W){let Me={...b,[D[i]]:B},oe=ie(Me,n);b=de({r:255-oe.r,g:255-oe.g,b:255-oe.b},n)}else b={...b,[D[i]]:B};B>m[D[i]]&&(m={...m,[D[i]]:B}),ee(),_(),L()},getRgbForCopy:()=>ie(b,n),onRandom:i=>ve(i),onReset:()=>ve({r:0,g:0,b:0})},n=o.mode??"rgb",u=o.initialColor?de(o.initialColor,n):{x:.7,y:.4,z:.85},m={x:1,y:1,z:1},b={...u},M=0,C=()=>o.alpha!==void 0,w=Math.max(0,Math.min(1,o.alpha??1));function x(i){let f=Math.max(0,Math.min(1,i));f!==w&&(w=f,ee(),_(),L())}function R(i){let f=ae(),v=fe(f);v.s=Math.max(0,Math.min(100,i*100));let E=Fe(v);ve(W?{r:255-E.r,g:255-E.g,b:255-E.b}:E)}let g=new Set;Wo();let p=document.createElement("div");p.className="box-picker";let k=document.createElement("canvas");k.style.cursor="grab",p.appendChild(k);let A=vo(k,t),z={...Ve},y=!0,P=document.createElement("button");P.className="box-axis-toggle-btn active",P.title="Toggle All Guides",P.setAttribute("aria-label","Toggle All Guides"),P.innerHTML='<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21h18"/><path d="M3 21V3"/><path d="M3 21l7-7"/><path d="M19 17l2 4-4-2"/><path d="M7 5l-4-2 2 4"/></svg>',P.addEventListener("click",i=>{i.stopPropagation(),y=!y,z={vertexX:y,vertexY:y,vertexZ:y,centerX:y,centerY:y,centerZ:y,yawArc:y,pitchArc:y},P.classList.toggle("active",y),L()}),p.appendChild(P);let I=null,$=document.createElement("div");$.className="box-picker-controls",I=document.createElement("div"),I.className="box-picker-swatch",$.appendChild(I),p.appendChild($),(l||a||c)&&To($,s,{showInputs:l,showModeToggle:a,showCorners:c}),e.appendChild(p);let Q=Vo(k,()=>m,i=>{m=i},()=>b,(i,f)=>{b=i,M=f,ee(),_()},()=>M,()=>A.scale,()=>A.center,L,C,x,()=>w,()=>Z(b,A.scale,A.center),R,()=>fe(ae()).s/100),W=!1;k.addEventListener("dblclick",()=>{W=!W,lo(W),ee(),_(),L()});function ne(i){if(i===n)return;let f=ie(b,n),v={...b},E={...m};n=i;let D=de(f,n),B={x:1,y:1,z:1};b=D,m=B,j(v,D,E,B,300),_()}let Y=null;function j(i,f,v,E,D){Y!==null&&cancelAnimationFrame(Y);let B=performance.now();function Me(oe){let we=oe-B,Re=Math.min(1,we/D),N=1-Math.pow(1-Re,3);b={x:i.x+(f.x-i.x)*N,y:i.y+(f.y-i.y)*N,z:i.z+(f.z-i.z)*N},m={x:v.x+(E.x-v.x)*N,y:v.y+(E.y-v.y)*N,z:v.z+(E.z-v.z)*N},ge(),ee(),Re<1?Y=requestAnimationFrame(Me):Y=null}Y=requestAnimationFrame(Me)}let ue=!1;function L(){ue||(ue=!0,requestAnimationFrame(()=>{ue=!1,ge()}))}function ge(){ko(A,m,b,M,n,Q.state,z,{active:Q.state.alphaMode,alpha:w,rgb:ae()})}function re(i,f,v){return Math.round(i+(f-i)*v)}function xe(i,f){let v=f>0?255:0,E=Math.abs(f);return ke({r:re(i.r,v,E),g:re(i.g,v,E),b:re(i.b,v,E)})}function le(i,f){let v=Ge(f)||{r:128,g:128,b:128},E=xe(v,.35),D=xe(v,0),B=xe(v,-.35);i.innerHTML=`<svg viewBox="0 0 48 48" width="64" height="64" aria-hidden="true"><defs><filter id="sw-mssllzpn" x="-60%" y="-60%" width="220%" height="220%"><feDropShadow dx="0" dy="1" stdDeviation="1.2" flood-color="#000" flood-opacity="0.32"/></filter></defs><g filter="url(#sw-mssllzpn)"><path d="M24 5 L42 14 L24 23 L6 14 Z" fill="${E}"/><path d="M6 14 L24 23 L24 45 L6 36 Z" fill="${D}"/><path d="M42 14 L24 23 L24 45 L42 36 Z" fill="${B}"/><path d="M24 23 L24 45 M6 14 L6 36 M42 14 L42 36" stroke="rgba(0,0,0,.18)" stroke-width="1" fill="none"/></g></svg>`,i.style.backgroundColor="transparent"}function pe(i){try{navigator.clipboard.writeText(i).catch(()=>{})}catch{}}function Do(i){i&&(i.style.borderColor="#4ade80",i.style.boxShadow="0 0 0 2px rgba(74,222,128,.35)",setTimeout(()=>{i.style.borderColor="",i.style.boxShadow=""},500))}function ae(){let i=ie(b,n);return W?{r:255-i.r,g:255-i.g,b:255-i.b}:i}function _(){if(!r)return;let i=ae(),f=ke(i);I&&le(I,f);let v=p.querySelector(".box-picker-hex");v&&(v.value=f),ye(),p._updateModeButtons&&p._updateModeButtons()}function ye(){if(!r)return;let i=qe[n],f=W?de(ae(),n):b,v=no(f,n),E=p.querySelectorAll(".box-picker-channel input"),D=p.querySelectorAll(".box-picker-channel label");for(let B=0;B<E.length;B++)D[B].textContent=i[B],D[B].style.color="",D[B].style.textShadow="none",E[B].value=String(v[B])}function ee(){let i=ae(),f={rgb:i,hsb:fe(i),oklch:Ee(i),hex:ke(i),alpha:w};for(let v of g)v(f)}function Oe(){let i=ie(b,n);return{rgb:i,hsb:fe(i),oklch:Ee(i),hex:ke(i)}}_(),ge();let ve=i=>{b=de(i,n),m={x:Math.max(m.x,b.x),y:Math.max(m.y,b.y),z:Math.max(m.z,b.z)};let f=Z(b,A.scale,A.center);M=-1;for(let v=q.length-1;v>=0;v--)if(he(v,f,m,A.scale,A.center)){M=v;break}ee(),_(),L()};return{getColor:Oe,getMode:()=>n,setColor:ve,setAlpha:x,getAlpha:()=>w,setMode(i){ne(i)},getRotation:()=>fo(),setRotation:(i,f)=>{mo(i,f),L()},getAxisRotation:()=>uo(),setAxisRotation:(i,f,v)=>{bo(i,f,v),L()},getGuides:()=>({...z}),setGuides:i=>{z={...z,...i},L()},toggleAllGuides:i=>{let f=i!==void 0?i:!y;y=f,z={vertexX:f,vertexY:f,vertexZ:f,centerX:f,centerY:f,centerZ:f,yawArc:f,pitchArc:f},P.classList.toggle("active",f),L()},setZoom:i=>{ho(i),L()},getZoom:()=>go(),setDimensions:(i,f,v)=>{xo(i,f,v),L()},getDimensions:()=>po(),getEdgeStyle:()=>Co(),setEdgeStyle:i=>{Mo(i),L()},on(i,f){g.add(f)},off(i,f){g.delete(f)},destroy(){Q.destroy(),Y!==null&&cancelAnimationFrame(Y),e.removeChild(p)}}}export{Le as DEFAULT_EDGE_CONFIG,Ve as DEFAULT_GUIDES,jo as createBoxColorPicker,Mt as createColorPicker,po as getBoxDimensions,fo as getCameraAnglesDeg,Co as getEdgeStyle,uo as getRotationDeg,go as getZoomMultiplier,xo as setBoxDimensions,lo as setBoxInvert,mo as setCameraAnglesDeg,Mo as setEdgeStyle,bo as setRotationDeg,ho as setZoomMultiplier};
